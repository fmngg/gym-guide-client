...
import { ExerciseDto, ExercisesDto } from '@/core/api/dto';
...
const Favourites: FC = () => {
  ...
  if (exercisesData && exercisesData.length === 0) {
	...
	C316.426,196.043,380.533,141.939,412.861,78.976z"
        ...
        <h1>Your favourites section is clear. Leave a like to see it here.</h1>
        <Button onClick={() => navigate('/exercises')}>To exercises</Button>
      </div>
    );
  }
  ...
  return (
    <div className={styles.favourites}>
      {exercisesData &&
        exercisesData.map((obj: ExerciseDto) => (
          <Link key={obj.id} to={`/exercises/${obj.id}`}>
            <Exercise isFavourite={true} exercise={obj} />
          </Link>
        ))}
    </div>
  );
